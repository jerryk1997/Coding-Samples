...
#Truncated list of cities in the world to only 10 countries
...
#Loop through all cities and search for tweets by city
#Retry on failure until last city is reached
while True:
    start_row = 0
    world_cities_df = world_cities_df.iloc[start_row: , :] 
    for index, row in world_cities_df.iterrows():
        ...
        try:
            #For each city search through all keywords
            ...
            #Add country to a list if there are scraped tweets for ease of moving files later
